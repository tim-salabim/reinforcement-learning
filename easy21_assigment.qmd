...
# while (min(dat$Na) <= 50) {
  ...
  if (exists("dat")) {
    ...
    act = chooseAction(epsilon(Ns), greedy_act)
  ...
  for (i in nrow(df_episode)) {
    ...
    df_episode[
      state_dealer == crd_dlr &
        state_player == sum_plr &
        action == act
      , value := mcUpdate(value, s1$r, Nsi, alpha(Na))
    ]
...
saveRDS(dat, "data/20231218_dat.rds")
...
png("imgs/MCcontrol.png", width = 800, height = 800, units = "px")
print(Q)
dev.off()
