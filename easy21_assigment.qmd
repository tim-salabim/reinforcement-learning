...
### FIXME here - but how?
playEpisode = function(SD) {
  SDout = copy(SD)
  s1 = step(SDout[, state[[1]]], SDout[, action])
  ...
  SDout = rbind(SDout, df)
  
  if (s1$terminated) {
    return(SDout)
  } else {
    return(playEpisode(SDout[nrow(SDout), ]))
  }
}
...
df = data.table(
  state = list("s" = c(dealers_card, players_cards))
  , action = chooseAction(epsilon(0))
  , Na = 0
  , Ns = 1
  , value = 0
